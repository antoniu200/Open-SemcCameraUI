...
public class MenuBuilder implements SupportMenu
{
    ...
    private static int getOrdering(final int n) {
        ...
        if (n2 >= 0 && n2 < MenuBuilder.sCategoryToOrder.length) {
            return (n & 0xFFFF) | MenuBuilder.sCategoryToOrder[n2] << 16;
        }
    ...
    public boolean collapseItemActionView(final MenuItemImpl menuItemImpl) {
        ...
        if (!empty && this.mExpandedItem == menuItemImpl) {
            this.stopDispatchingItemsChanged();
            ...
            while (true) {
                ...
                if (menuPresenter == null) {
                    ...
                    b = (collapseItemActionView = menuPresenter.collapseItemActionView(this, menuItemImpl));
                    if (b) {
                        break;
                    }
                    continue;
    ...
    void findItemsWithShortcutForKey(final List<MenuItemImpl> list, final int n, final KeyEvent keyEvent) {
        ...
        for (int size = this.mItems.size(), i = 0; i < size; ++i) {
            ...
            if ((modifiers & 0x1100F) == (n2 & 0x1100F) && c != '\0' && (c == keyCharacterMap$KeyData.meta[0] || c == keyCharacterMap$KeyData.meta[2] || (qwertyMode && c == '\b' && n == 67)) && menuItemImpl.isEnabled()) {
