...
public class LruCache<K, V> {
    ...
    public void trimToSize(int maxSize) {
        while (true) {
            ...
            synchronized (this) {
                ...
                Map.Entry<K,V> e = map.entrySet().iterator().next(); // map.eldest();
                if (toEvict == null) {
                    break;
                }
                key = toEvict.getKey();
    ...
    public final synchronized String toString() {
        ...
        return String.format(Locale.US, "LruCache[maxSize=%d,hits=%d,misses=%d,hitRate=%d%%]", Integer.valueOf(this.maxSize), Integer.valueOf(this.hitCount), Integer.valueOf(this.missCount), Integer.valueOf(i != 0 ? (100 * this.hitCount) / i : 0));
    }
}
