    ...
    void applyTransformation(float f, Ring ring, boolean z) {
        ...
        if (f != 1.0f || z) {
            ...
            if (f < SHRINK_OFFSET) {
                ...
                float interpolation2 = (0.79f * MATERIAL_INTERPOLATOR.getInterpolation(f2)) + MIN_PROGRESS_ARC + startingStartTrim2;
                interpolation = startingStartTrim2;
                startingStartTrim = interpolation2;
            } else {
                float f3 = (f - SHRINK_OFFSET) / SHRINK_OFFSET;
                startingStartTrim = ring.getStartingStartTrim() + 0.79f;
                interpolation = startingStartTrim - ((0.79f * (1.0f - MATERIAL_INTERPOLATOR.getInterpolation(f3))) + MIN_PROGRESS_ARC);
            }
            float f4 = startingRotation + (RING_ROTATION * f);
            float f5 = GROUP_FULL_ROTATION * (f + this.mRotationCount);
